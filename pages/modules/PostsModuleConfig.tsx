...
export const PostsModuleConfig: React.FC = () => {
  ...
  // Comment Fields (shown when enableComments is true)
  const [commentFields, setCommentFields] = useState<FieldConfig[]>([
    ...
    { id: 'CM2', name: 'Thứ tự', key: 'order', type: 'text', required: true, enabled: true, isSystem: true },
    { id: 'CM3', name: 'Trạng thái', key: 'active', type: 'boolean', required: true, enabled: true, isSystem: true },
    // Optional
    { id: 'CM4', name: 'Tác giả', key: 'author', type: 'text', required: false, enabled: true, isSystem: false },
  ]);
  ...
  const handleToggleCommentField = (id: string) => {
  ...
  const requiredCommentFields = commentFields.filter(f => f.required);
  const optionalCommentFields = commentFields.filter(f => !f.required);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {[
                { key: 'enableComments', label: 'Bình luận', icon: MessageSquare },
      ...
      {/* Comment Fields - Dynamic, shown when enableComments is true */}
      {features.enableComments && (
        <div className="bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800/50 rounded-lg p-4">
          <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3 flex items-center gap-2">
            <MessageSquare size={14} className="text-amber-500" /> Trường bình luận
          </h3>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {requiredCommentFields.map(field => (
              <FieldRow key={field.id} field={field} onToggle={handleToggleCommentField} color="cyan" />
            ))}
            {optionalCommentFields.map(field => (
              <FieldRow key={field.id} field={field} onToggle={handleToggleCommentField} color="cyan" />
            ))}
          </div>
        </div>
